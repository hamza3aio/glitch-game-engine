...
Tests: `npm test` (vitest, 144 tests: math, ECS, hierarchy, IDs, prefabs,
...
samples/synth, particles, terrain, navigation/AI, editor/history/gizmo/DOM).
...
- `src/ai/` — `navmesh.ts` (baked walkability grids, A* + smoothing, dynamic obstacles), `agent.ts` (waypoint follow, separation)
...
- `src/fx/` — `particles.ts` (pooled CPU particles: emission, bursts, gravity/drag/bounce, color/size over life, follow, cone sampling)
